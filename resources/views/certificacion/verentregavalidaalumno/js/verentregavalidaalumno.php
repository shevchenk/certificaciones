...
$(document).ready(function() {
    ...
    $('#ModalVerentregavalidaalumno').on('shown.bs.modal', function (event) {
        ...
        if( AddEdit==1 ){        
            ...
            $("#ModalVerentregavalidaalumnoForm").append("<input type='hidden' value='"+BandejaG.id+"' name='certificado_id'>");
        }
        $('#ModalVerentregavalidaalumnoForm #txt_ode').val( BandejaG.ode );
        $('#ModalVerentregavalidaalumnoForm #txt_dni').val( BandejaG.dni );
        $('#ModalVerentregavalidaalumnoForm #txt_nombre').val( BandejaG.nombre );
        $('#ModalVerentregavalidaalumnoForm #txt_tramite').val( BandejaG.tramite );
        $('#ModalVerentregavalidaalumnoForm #txt_fecha_tramite').val( BandejaG.fecha_tramite );
        $('#ModalVerentregavalidaalumnoForm #txt_fecha_ingreso').val( BandejaG.fecha_ingreso );
    });

    $('#ModalVerentregavalidaalumno').on('hidden.bs.modal', function (event) {
        $("#ModalVerentregavalidaalumnoForm input[type='hidden']").not('.mant').remove();
        $("#ModalVerentregavalidaalumnoForm #slct_estado_contesto").selectpicker('val','');
        $("#ModalVerentregavalidaalumnoForm #slct_contesta_id").selectpicker('val','0');
        $("#ModalVerentregavalidaalumnoForm #txt_observacion").val('');
    });
    
    $( "#ModalVerentregavalidaalumno #slct_estado_contesto" ).change(function() {
            if ($( "#ModalVerentregavalidaalumno #slct_estado_contesto" ).val()==1) {
                $('#ModalVerentregavalidaalumno #respuesta').css('display',"");
            }
            else {
                $('#ModalVerentregavalidaalumno #respuesta').css('display',"none");
            }
    });
});
...
AgregarEditar=function(val,id){
    ...
    $('#ModalVerentregavalidaalumno').modal('show');
}
...
SlctCargarContesta=function(result){
    ...
    $("#ModalVerentregavalidaalumno #slct_contesta_id").html(html); 
    $("#ModalVerentregavalidaalumno #slct_contesta_id").selectpicker('refresh');
...
HTMLAgregarEditar=function(result){
    if( result.rst==1 ){
        msjG.mensaje('success',result.msj,4000);
        $('#ModalVerentregavalidaalumno').modal('hide');
        AjaxBandeja.Cargar(HTMLCargarBandeja);
    }else{
...
ValidaForm=function(){
    var r=true;
    if( $.trim( $("#ModalVerentregavalidaalumnoForm #slct_estado_contesto").val() )=='' ){
        r=false;
        msjG.mensaje('warning','Seleccione si Contestó el Alumno',4000);
    }
    else if( $.trim( $("#ModalVerentregavalidaalumnoForm #slct_estado_contesto").val() )=='1' && $.trim( $("#ModalVerentregavalidaalumnoForm #slct_contesta_id").val() )=='0' ){
        r=false;
