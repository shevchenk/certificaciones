...
CambiarEstado=function(id){
    sweetalertG.confirm("Confirmación!", "Confirme su Validación del Alumno", function(){
        AjaxBandeja.CambiarEstado(HTMLCambiarEstado,id);
    });
}

CambiarEstadoMasivo=function(){
    sweetalertG.confirm("Confirmación!", "Confirme su envio Masivo a Validación del Alumno", function(){
...
HTMLCargarBandeja=function(result){ //INICIO HTML
    ...
    $.each(result.data.data,function(index,r){ //INICIO FUNCTION
        boton='<td> <a class="btn btn-success btn-sm" onClick="CambiarEstado('+r.id+')"><i class="fa fa-check fa-lg">Confirmar<br>Entrega y Validación del Alumno</i></a> </td>'; 
