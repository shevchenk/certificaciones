...
HTMLCargarEspecialidad=function(result){
    ...
    $.each(result.data.data,function(index,r){
        ...
        if( r.tipo==1 ){
            ...
            cuota="<ol><li>C - "+$.trim(r.cronograma2).split("|").join("</li><li>C - ")+"</li></ol>";
        }
        if( $.trim(r.adicional) != '' ){
        ...
        btncheck = '<td><span class="btn btn-primary btn-sm" onClick="SeleccionarEspecialidad(\''+r.id+'\',this)"+><i class="glyphicon glyphicon-ok"></i></span></td>';
            ...
            "<td class='curso col-md-5'>"+
                "<table class='table table-bordered table-hover'>"+
                    "<thead><tr>"+
                        ...
                        "<th>Nota</th>"+*/
                    "</tr></thead>"+
                    "<tbody id='table_"+r.id+"'><tr>"+
                        "<td>"+$.trim(r.cursos).split('|').join('</td><td>').split("^^").join("</td>"+btncheck+"</tr><tr><td>")+btncheck+"</td>"+
                    "</tr></tbody>"+
                "</table>"+
            "</td>";
        html+="</tr>";
...
SeleccionarEspecialidad=function(id, t){
    a = $(t).parent().parent().addClass('mant').parent();
    $("#table_"+id+" tr").not(".mant").remove();
