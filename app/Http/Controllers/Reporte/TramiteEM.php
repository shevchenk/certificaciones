...
use App\Models\Reporte\Tramite;
use Excel;

class TramiteEM extends Controller
{
    ...
    {
        if ( $r->ajax() ) {
            $renturnModel = Tramite::runLoad($r);
            $return['rst'] = 1;
    ...
    {
        $renturnModel = Tramite::runExport($r);
        
        Excel::create('Tramites', function($excel) use($renturnModel) {

        $excel->setTitle('Reporte de Tramites')
              ->setCreator('Jorge Salcedo')
              ->setCompany('JS Soluciones')
              ->setDescription('Matrícula PAE o Seminarios');

        $excel->sheet('Tramites', function($sheet) use($renturnModel) {
            ...
            $sheet->cell('A1', function($cell) {
                $cell->setValue('REPORTE DE TRAMITES');
                $cell->setFont(array(
