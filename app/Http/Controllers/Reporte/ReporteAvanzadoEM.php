...
class ReporteAvanzadoEM extends Controller
{
    ...
    public function LoadVendedorComision(Request $r )
    {
        if ( $r->ajax() ) {
            $url=explode("/",$_SERVER['HTTP_REFERER']);
            $renturnModel = ReporteAvanzado::runVendedorComision($r);
            $return['rst'] = 1;
            $return['data'] = $renturnModel;
            $return['msj'] = "No hay registros aún";
            return response()->json($return);
    ...
    public function ExportVendedorComision(Request $r )
    {
        $url=explode("/",$_SERVER['HTTP_REFERER']);
        $renturnModel = ReporteAvanzado::runExportVendedorComision($r);

        Excel::create('Vendedor Comisión', function($excel) use($renturnModel) {

        $excel->setTitle('Reporte de Seminarios')
              ->setCreator('Jorge Salcedo')
              ->setCompany('JS Soluciones')
              ->setDescription('Vendedor Comisión');

        $excel->sheet('Vendedor Comisión', function($sheet) use($renturnModel) {
            $sheet->setOrientation('landscape');
            $sheet->setPageMargin(array(
                0.25, 0.30, 0.25, 0.30
            ));
            ...
            $sheet->cell('A1', function($cell) use ($renturnModel) {
                $cell->setValue('PLANILLA DE COMISIÓN - PERSONA MARKETING - MES:'.$renturnModel['mes']);
                $cell->setFont(array(
                    'family'     => 'Bookman Old Style',
                    'size'       => '21',
                    'bold'       =>  true
